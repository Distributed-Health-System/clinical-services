...
export class KeycloakAdminService {
  ...
  async createPatientUser(
    ...
    if (!userId) {
      throw new InternalServerErrorException('Could not extract user ID from Keycloak response');
    }

    await this.assignPatientRole(userId, token);

    this.logger.log(`Keycloak user created: ${userId} (${email})`);
    return userId;
  }

  private async assignPatientRole(userId: string, token: string): Promise<void> {
    const roleRes = await fetch(`${this.baseUrl}/roles/patient`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    if (!roleRes.ok) {
      throw new InternalServerErrorException('Could not find patient role in Keycloak');
    }

    const role = (await roleRes.json()) as { id: string; name: string };

    const assignRes = await fetch(`${this.baseUrl}/users/${userId}/role-mappings/realm`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    if (!assignRes.ok) {
      ...
      this.logger.error(`Failed to assign patient role: ${body}`);
